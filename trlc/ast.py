...
# Copyright (C) 2024-2025 Florian Schanda
#
...
class Check(Node):
    ...
    def perform(self, mh, composite_object, gstab):
...
class Expression(Node, metaclass=ABCMeta):
    ...
    def evaluate(self, mh, context, gstab):  # pragma: no cover
        ...
        The global symbol table must be None (for static context
        evaluations), otherwise it must contain the global symbol
        table to resolve record references.
...
class Record_Reference(Expression):
    ...
    def evaluate(self, mh, context, gstab):
        ...
        assert context is None or isinstance(context, dict)
        assert gstab is None or isinstance(gstab, Symbol_Table)
        return Value(self.location, copy(self.target.field), self.typ)
...
class Conditional_Expression(Expression):
    ...
    def evaluate(self, mh, context, gstab):
        ...
        return self.else_expr.evaluate(mh, context, gstab)
    ...
    def perform_type_checks(self, mh, value, gstab):
        ...
        if isinstance(value, Tuple_Aggregate):
            ...
            for check in self.iter_checks():
                if not check.perform(mh, value, gstab):
                    ok = False
            return ok
        else:
            assert isinstance(value, Implicit_Null)
...
class Record_Object(Typed_Entity):
    ...
    * :class:`Implicit_Null`
    ...
    def perform_checks(self, mh, gstab):
        # lobster-trace: LRM.Check_Evaluation_Order
        ...
        for check in self.n_typ.iter_checks():
            ...
            if not check.perform(mh, self, gstab):
                ok = False
